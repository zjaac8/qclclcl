...
Docker()
{
	...
	Region $R
	echo "{\"family\":\"$RandStr\",\"networkMode\":\"awsvpc\",\"cpu\":\"256\",\"memory\":\"512\",\"requiresCompatibilities\":[\"EC2\",\"FARGATE\"],\"containerDefinitions\":[{\"name\":\"$RandStr7\",\"image\":\"$IMAGE\",\"cpu\":1,\"command\":[\"start\",\"accept\",\"--token\",\"ayDMa3ja408jPdTBBzbTE52Mv9uWZ+QK963WOq7QVb4=\"],\"memoryReservation\":64,\"essential\":true},{\"name\":\"RandStr4\",\"image\":\"$IMAGE2\",\"cpu\":64,\"memoryReservation\":128,\"essential\":true,\"environment\":[{\"name\":\"RP_EMAIL\",\"value\":\"zhifengaac@gmail.com\"},{\"name\":\"RP_API_KEY\",\"value\":\"95f74a11-5098-44eb-9ac0-bce88683376c\"}]}]}" > ~/docker.json
	aws ecs create-cluster \
        --cluster-name $RandStr4
	...
	aws ecs create-service --cli-input-json "{\"capacityProviderStrategy\":[{\"base\":0,\"capacityProvider\":\"FARGATE\",\"weight\":1}],\"cluster\":\"$RandStr4\",\"serviceName\": \"$RandStr5\",\"deploymentConfiguration\":{\"maximumPercent\":200,\"minimumHealthyPercent\":100},\"desiredCount\":121,\"networkConfiguration\":{\"awsvpcConfiguration\":{\"assignPublicIp\":\"ENABLED\",\"securityGroups\":[\"$GID\"],\"subnets\":[\"$SID\"]}},\"taskDefinition\":\"$RandStr\"}"
	##sleep 3s
	aws ecs create-service --cli-input-json "{\"capacityProviderStrategy\":[{\"base\":0,\"capacityProvider\":\"FARGATE_SPOT\",\"weight\":1}],\"cluster\":\"$RandStr4\",\"serviceName\": \"$RandStr6\",\"deploymentConfiguration\":{\"maximumPercent\":200,\"minimumHealthyPercent\":100},\"desiredCount\":121,\"networkConfiguration\":{\"awsvpcConfiguration\":{\"assignPublicIp\":\"ENABLED\",\"securityGroups\":[\"$GID\"],\"subnets\":[\"$SID\"]}},\"taskDefinition\":\"$RandStr\"}"
	aws ec2 describe-network-interfaces | jq .NetworkInterfaces[].PrivateIpAddresses[].Association.PublicIp | sed 's/"//g' > ~/aws/$R
	if [ "$(cat ~/aws/$R |wc -l)" -lt "25" ];then
		#sleep 3s
		Docker $1
	fi
}
